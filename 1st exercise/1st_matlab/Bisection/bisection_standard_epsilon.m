...
lamda_values = linspace(0.005,0.1,50);                               % και θέτουμε το lamda_values = linspace ώστε το λ να πάρει διάφορες τιμες
results = zeros(50,3);                                               % ορίζουμε έναν πίνακα results με μέγεθος όσες οι τιμές του λ επί 3 που ειναι οι 3 συναρτησεις
functions = {f1, f2, f3};                                            %ορίζουμε έναν πίνακα functions στοιχεία τις 3 συναρτήσεις μας                 
...
for i = 1:3                                                          % loop απο 1 εως 3 για να κάνει interate και στις 3 συναρτήσεις
    for j = 1:length(lamda_values)                                   % loop για να περάσουμε από όλες τις τιμές του λ που θα μας δώσει το linspace 
       ...
       [a, b, k, ~, l] = mybisection(functions{i}, lamda, epsilon);          % καλούμε τη συνάρτηση mybisection
...
title('Bisection Method: Number of Iterations vs lamda')
legend('Function 1');
grid on;
