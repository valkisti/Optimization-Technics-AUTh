...
epsilon = 0.001;                                                 % θέτουμε το ε=0.001
lamda_values = [0.01, 0.15, 0.5];                                % και ορίζουμε τον πίνακα lamda_values ο οποίος έχει για στοιζεία του 3 τιμές για το λ
    ...
    for j = 1:length(lamda_values)                               % loop για να περάσουμε από όλες τις τιμές του λ που περιέχει ο πίνακας lamda_values
        ...
        [a, b, k, e, l] = mybisection(f1, lamda, epsilon);       % καλούμε τη συνάρτηση mybisection
        ...
        xlabel('Iterations (k)');
        ylabel('[a(k), b(k)]');
        title('Bisection Method: "a" and "b" values in f1');
        grid on;
        legend("Values of a and b for l=0.01"," ","Values of a and b for l=0.15"," ","Values of a and b for l=0.5"," ");
    end

     for j = 1:length(lamda_values)                                     % όμοια με την 1η συνάρτηση
        lamda = lamda_values(j);
        [a, b, k, e, l] = mybisection(f2, lamda, epsilon);
        ...
        xlabel('Iterations (k)');
        ylabel('[a(k), b(k)]');
        title('Bisection Method: "a" and "b" values in f2');
        grid on;
        legend("Values of a and b for l=0.01"," ","Values of a and b for l=0.15"," ","Values of a and b for l=0.5"," ");
    end
     ...
     for j = 1:length(lamda_values)                                          % όμοια με την 1η συνάρτηση
        lamda = lamda_values(j);
        [a, b, k, e, l] = mybisection(f3, lamda, epsilon);
        ...
        xlabel('Iterations (k)');
        ylabel('[a(k), b(k)]');
        title('Bisection Method: "a" and "b" values in f3');
        grid on;
        legend("Values of a and b for l=0.01"," ","Values of a and b for l=0.15"," ","Values of a and b for l=0.5"," ");
     end