function [k,d, x, y] = lavenberg_method_armijo(f, x_0, y_0,e)
    syms x y;

    df_dx = diff(f, x);                                    %υπολογισμός των παραγώγων
    df_dy = diff(f, y);
    hessian = [diff(df_dx, x), diff(df_dx, y); diff(df_dy, x), diff(df_dy, y)];                      %υπολογίσμος τον γενικό τύπο του εσσιανού πίνακα
    ...
    x = x_0;
    y = y_0;
    max_k = 100;                  %ορίζω μέγιστο αριθμό επαναλήψεων
    k = 1 ;            
    
    s = 1;                       %ορίζω τις παραμέτρους του κανόνα Armijo
    a=0.001;
    b=0.2;
    ...
    while k <= max_k
      ...
      hessian_val = hessian_f(x(k), y(k));         % ορίζω την μεταβλητή hessian_val και της θέτω την τιμή του gradient για συγκεκριμένα χ και y
        ...
        f_k = f(x(k),y(k));                                            %υπολογίζω τις τιμές f(x(k)) και f(x(k+1))
        f_k1 = f(x(k) - s * grad_val(1), y(k) - s * grad_val(2));
        m=0;
        while f_k - f_k1 < -a*b^m*s*d'*grad_val                        % και υπολογίζω το m έτσι ώστε να τηρείται το κριτήριο 4
            m=m+1;
        end
