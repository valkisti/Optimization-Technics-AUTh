...
[k,~,xk,yk] = steepest_descent(f, x_0, y_0, e, 1);
k
figure;
...
title({'Steepest Descent method','for constant g = 1','starting at (-1,-1)'})
xlabel('x') 
ylabel('y') 
grid on;
...
[k,~, xk, yk] = steepest_descent_minimize_g(f, x_0, y_0,e);
k
figure;
[X,Y]=meshgrid(-2.5:0.1:2.5);
Z= X.^3 .* exp(-X.^2-Y.^4);
...
hold on;
plot(xk, yk, 'Color', 'r', 'Marker','o');
title({'Steepest Descent method','for g value based on golden section method','starting at (-1,-1)'})
xlabel('x') 
ylabel('y') 
grid on;
...
[k,~,~, xk, yk] = steepest_descent_armijo(x_0, y_0,f, e);
k
...
hold on;
plot(xk, yk, 'Color', 'r', 'Marker','o');
title({'Steepest Descent method','for g based on Armijo Rule','starting at (-1,-1)'})
xlabel('x') 
ylabel('y') 
grid on;
